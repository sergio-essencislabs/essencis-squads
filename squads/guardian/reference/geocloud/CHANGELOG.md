...
- [ADR-0007](knowledge/decisions/0007-fluentmigrator-para-migrations.md): GeoCloud usa FluentMigrator 6.2 (`M001`…`M027`, runner no startup). Supersede ADR-0004.
- [KI-0007](knowledge/known-issues/0007-i18n-literais-hardcoded.md): dívida de `uiText`/`LITERALS` incompletos (canônico no produto: `Documentation/Hallucination/known-issues/0001-i18n-literais-hardcoded.md`).

### Alterado

- Pasta canônica da documentação estrutural: `Documentation/<Pasta>/` (uma pasta por branch, na raiz do repositório do produto). Policies `documentacao`/`orquestracao` e skills `documentation-sync`/`structural-spreadsheet-sync`/`uml-generator` apontam para esse caminho — não mais a raiz de `Documents\GeoCloud\`, `.cursor/skills/Documentation/` nem o espelho em Miscelaneous.
- **Caminhos de produto passam a ser relativos ao repositório** em `policies/`, `skills/` e `agents/` — os arquivos que `sync-cursor.ps1` copia para dentro do produto. Caminho absoluto nesses arquivos já quebrou duas vezes (mudança `Documents\` → `C:\Software\`, e renomeação `GeoCloudAI_Replit` → `GeoCloudAI`), e cada quebra é silenciosa: o agente lê a instrução e grava no lugar errado, ou em lugar nenhum.
- `scripts/sync-cursor.ps1`: alvo único atualizado para `GeoCloud\GeoCloudAI`; a variável da raiz dos repositórios deixou de se chamar `$documentsRoot` — o framework não vive mais em `Documents`.
- Repositório do produto renomeado de `GeoCloudAI_Replit` para `GeoCloudAI`, igualando o nome do remote (`Essencis-Labs/GeoCloudAI`). Referências atualizadas em policies, skills, agents, knowledge, playbooks, scripts, README, MASTER_PROMPT e FRAMEWORK_ARCHITECTURE.
- Corrigido nome de produto duplicado (``GeoCloudAI_Replit`` aparecendo duas vezes onde eram dois produtos distintos), introduzido por find/replace cego no fork 1.0.0 (`b0f0460`, "produto unico"), em `FRAMEWORK_ARCHITECTURE.md`, `MASTER_PROMPT.md`, `knowledge/architecture/visao-geral.md` e `playbooks/onboarding-nova-base.md`.
- Policy `banco-de-dados`, playbook `migration` e `agents/database-architect.md` descrevem o runner FluentMigrator real (ADR-0007), não mais "script SQL manual / sem ferramenta".
- ADR-0004 marcado como superado; KI-0005 permanece resolvida, agora via ADR-0007.
...
- `devops-postgres-agent`: obsoleto por inteiro após o [ADR-0006](knowledge/decisions/0006-geocloud-usa-mysql.md).
...
**`geocloud-ai-framework`**, exclusivo do GeoCloud. O escopo cross-produto foi removido,
...
- **Premissa de banco corrigida: PostgreSQL → MySQL 8.** A documentação afirmava que o
  GeoCloud rodava sobre PostgreSQL/Npgsql. A verificação no código do produto mostrou
  `MySqlConnector`, `SELECT LAST_INSERT_ID()` e dump `MySQL 8.0.46` — a migração havia
  acontecido sem que a documentação acompanhasse. Ver [ADR-0006](knowledge/decisions/0006-geocloud-usa-mysql.md).
...
- `scripts/sync-cursor.ps1`: alvo único, `GeoCloud/GeoCloudAI_Replit`.
- Referências internas de `ai-framework` para `geocloud-ai-framework` (45 ocorrências).
...
- Referências operacionais a essas bases em README, arquitetura, playbooks, policies, skills, scripts
  ...
  `GeoCloud/GeoCloudAI_Replit` e `ELIMS/ELIMS_Replit`.

### Modificado

- `scripts/sync-cursor.ps1`: `$deployTargets` agora aponta para
  `Documents/GeoCloud/GeoCloudAI_Replit` e `Documents/ELIMS/ELIMS_Replit`.
...
- `ADR-0006` (removido na 1.0.0): `ELIMS` adota **MySQL** como motor
  de banco de dados oficial (driver `MySqlConnector`), divergindo intencionalmente do GeoCloud
...
- Porta completa da branch `elims-geocloud-padronization` (`ELIMS/ELIMS_Replit`, fonte read-only) para
...
- `ELIMS/ELIMS_Replit/docs/Classe-entitydetail-proposta-de-normalizacao.md`: comparado com o estado real de
...
- `.xlsx` de `ELIMS/ELIMS_Replit/docs/estrutura/Account_ELIMS_resumo_estrutural.xlsx` não regenerado a partir do
...
- **Modelo de publicação revogado:** o repositório de "publicação consolidada" (`sergio-essencislabs/TESTS`,
  via `git subtree`) foi abandonado. `ai-framework/`, `GeoCloud/` e `ELIMS/` agora têm cada um seu
  próprio repositório remoto privado e dedicado no GitHub (`sergio-essencislabs/ai-framework`,
  `sergio-essencislabs/GeoCloud`, `sergio-essencislabs/ELIMS`), para uso com cada projeto aberto em
...
- Campanha de testes de permissão/segurança V7 para o ELIMS (`TestCenter/Elims/`), replicando e
  ampliando a metodologia já usada no GeoCloud (`TestCenter/GeoCLoud/`, arquivos originalmente em
  `GeoCloud/Miscelaneous/` centralizados e renomeados para o padrão `_V7` no final do nome). Ferramenta
...
- 4 Known Issues novos, todos no ELIMS, encontrados pela campanha V7:
  ...
  - **KI-0010 (alta):** `Entity.GetByUser` sem guarda de tenant (IDOR) — a correção já aplicada no
    GeoCloud não foi replicada no ELIMS.
  - **KI-0011 (média):** JWT sem claim `accountId` derruba `ControllerBaseMiddleware` com
    `NullReferenceException` (500 em vez de 401) — recorrência do bug já catalogado no GeoCloud.
- `ELIMS/ELIMS_Replit/backend/src/Back.ApiTests/` e `Back.IntegrationTests/`: projetos xUnit novos (fuzzing de
...
- `GeoCloud/GeoCloudAI_Replit/db/geocloudai.sql` (+ `db/README.md`): cópia de trabalho do dump `pg_dump` (texto,
  schema+dados) do banco do GeoCloud, capturado de `GeoCloud/GeoCloudDB/geocloudai.sql` — hash SHA-256
  verificado idêntico à origem.
- `ELIMS/ELIMS_Replit/db/elims.sql` (+ `db/README.md`): cópia de trabalho do dump `pg_dump` formato custom
...
- Convenção documentada nos dois `db/README.md`: a partir de agora, `GeoCloud`/`ELIMS` trabalham
  com estas cópias locais, não com as pastas originais (`GeoCloudDB`/`ELIMS DB`), que permanecem
  intocadas como fonte histórica — mesma regra já aplicada ao código-fonte (`GeoCloudAI_Replit`/`ELIMS_Replit`).
...
- `ELIMS/ELIMS_Replit/frontend/angular.json`: orçamento `anyComponentStyle` alinhado ao de `GeoCloud/web`
...
- `dotnet build` de `GeoCloud/GeoCloudAI_Replit/api/Back.sln` e `ELIMS/ELIMS_Replit/backend/src/Back.sln`: sucesso, 0 erros (apenas
  avisos pré-existentes de NuGet — `MailKit`/`MimeKit`/`AutoMapper` — e nullability).
- `npm run build` (`ng build`, produção) de `GeoCloud/web` e `ELIMS/frontend`: sucesso, 0 erros, após
  a correção acima.
...
- Skill `structural-spreadsheet-sync` (19ª skill) para manter as planilhas estruturais canônicas de
  GeoCloud/ELIMS sincronizadas com o código.
...
- `scripts/verify-sources-untouched.ps1`: baseline + verificação de hash/tamanho/data para garantir
  que `GeoCloud/GeoCloudAI_Replit` e `ELIMS/ELIMS_Replit` permanecem intocados durante qualquer tarefa.
- `scripts/csv-to-xlsx.py`: converte o `.csv` multi-aba (marcadores `===== SHEET: X =====`) das
  planilhas estruturais em `.xlsx` espelho.
- `GeoCloud/GeoCloudAI_Replit/docs/estrutura/Account_GeoCloud_resumo_estrutural.csv`/`.xlsx` e
  `ELIMS/ELIMS_Replit/docs/estrutura/Account_ELIMS_resumo_estrutural.csv`/`.xlsx`: planilhas estruturais
  completas (todas as classes/endpoints) dos dois produtos, extraídas manualmente do código-fonte.
- `ELIMS/ELIMS_Replit/backend/docs/migrations.md`: documento de processo de schema criado desde o início
  (evita a mesma divergência doc×código encontrada no GeoCloud).

### Corrigido

- `GeoCloud/GeoCloudAI_Replit/api/docs/migrations.md`: reescrito para refletir a realidade do código (sem runner
...
- `ELIMS/Miscelaneous/Account_ELIMS_resumo_estrutural.csv`/planilha antiga — substituída pela versão
  canônica completa em `ELIMS/ELIMS_Replit/docs/estrutura/`.
...
- Scaffold inicial de `GeoCloud/GeoCloudAI_Replit` (baseline de `GeoCloud/GeoCloudAI_Replit`) e `ELIMS/ELIMS_Replit` (baseline de `ELIMS/ELIMS_Replit`).
- ADR-0001, ADR-0002, ADR-0003 registrando as decisões fundacionais do framework.

### Baseado em

Descoberta completa de `GeoCloud`, `ELIMS`, `GeoLims` e do documento `ELIMS_GeoCloud_Visao_Tecnica.md` (28/07/2026), incluindo mapeamento de arquitetura, dívida técnica conhecida e ativos de documentação/knowledge já existentes.
