<#
.SYNOPSIS
  Sincroniza geocloud-ai-framework/policies e geocloud-ai-framework/skills (fonte versionada) para
  ...
  de CADA UM deles (e não mais só em GeoCloud/GeoCloudAI/.cursor), para que o Cursor
...
.EXAMPLE
  pwsh ./sync-cursor.ps1 -DeployTarget "C:\Software\GeoCloud\GeoCloudAI"
#>
param(
    [string[]]$DeployTarget
)
...
# vez, e um caminho fixo aqui falha em silencio (o Test-Path abaixo apenas
# avisa e pula, entao o sync "passa" sem ter copiado nada).
...
# consumir o geocloud-ai-framework.
$deployTargets = @(
    (Join-Path $reposRoot "GeoCloud\GeoCloudAI")
)
...
function Sync-CursorTarget {
    ...
    Get-ChildItem -Path $agentsDir -Filter "*.md" | ForEach-Object {
        ...
        $relativePath = "geocloud-ai-framework/agents/$($_.Name)"
        $wrapper = @"
---
name: $skillName
description: Assume a persona "$agentTitle" do framework GeoCloud. Use quando o usuário pedir explicitamente para agir como este agente, ou quando outro agente encaminhar uma tarefa para esta persona.
