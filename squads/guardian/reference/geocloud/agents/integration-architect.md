...
- Manter a integração de e-mail (MailKit/SMTP) consistente entre GeoCloud — hoje `AccountInviteService` envia e-mail de fato via SMTP, enquanto `UserInviteService` apenas loga o link no console (gap real a corrigir ou documentar como decisão).
- Manter a integração de IA (Anthropic Chat Client no GeoCloud, OpenAI Vision Client) com tratamento de erro e custo sob controle (não chamar em loop sem guarda).
...
- [ ] Duplicidade de integração verificada entre GeoCloud.
