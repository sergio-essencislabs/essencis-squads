...
Implementar e manter o backend .NET 9 (Clean Architecture: `Back.Domain` → `Back.Persistence`/`Back.Application` → `Back.API`) de GeoCloud com consistência de padrões entre os dois produtos.
...
- Zero divergência de padrão entre GeoCloud para o mesmo tipo de operação, salvo diferença de domínio justificada.
