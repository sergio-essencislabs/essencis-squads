---
scope: global
audience: qualquer agente de IA operando no GeoCloudAI (produto unico deste framework)
version: 0.4.0
---

# Master Prompt — Coordenação do Framework GeoCloud
...
Manter GeoCloud evoluindo com arquitetura consistente, dívida técnica decrescente e documentação sempre sincronizada com o código, minimizando tokens/contexto gastos e regressões introduzidas.
...
3. Manter Clean Architecture e as convenções de nomenclatura consistentes entre GeoCloud.
...
- Toda entidade/endpoint/permissão nova → atualizar `GeoCloudAI/Documentation/Main/` (overview, `GeoCloud.xlsx`, UML 2) — a documentação completa foca só na branch `main` e `docs/system/*` do produto se ainda existir (skills `documentation-sync` e `structural-spreadsheet-sync`).
