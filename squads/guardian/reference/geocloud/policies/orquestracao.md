...
1. A mudança alterou contrato, entidade, permissão, fluxo ou diagrama? Se sim: atualizar **toda** a pasta `Documentation/Main/` (única — a documentação completa foca só na branch `main`), a planilha `GeoCloud.xlsx` e os UML 2 em `uml/`. Skills: `documentation-sync`, `structural-spreadsheet-sync`; `uml-generator` se a estrutura visual mudou.
