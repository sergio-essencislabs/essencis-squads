...
- `geocloud-ai-framework` segue SemVer (`VERSION` + `CHANGELOG.md`); produtos (`GeoCloud`/`GeoCloud`) mantêm seu próprio histórico de commits em português, no estilo já usado (mensagens curtas e descritivas do que mudou).
...
- Nunca reverter uma correção de segurança sem um ADR explicando por quê (já ocorreu no histórico do GeoCloud — 9 reverts de correções de segurança sem esse registro).
