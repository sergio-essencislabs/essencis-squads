...
Síntese da descoberta (Fase 1). Fonte original: `Documents/GeoCloud/Miscelaneous/Markdown Files/ELIMS_GeoCloud_Visao_Tecnica.md`. Detalhes de cada produto ficam em `docs/system/README.md` do respectivo produto — este documento só cobre o que é comum.
...
- Banco: MySQL (`geocloudai` para GeoCloud, `geocloudai`), tabelas/colunas minúsculas sem aspas, FK por convenção de nome.
- Testes: xUnit + FluentAssertions + NSubstitute (unitário), MySQL real efêmero sem Docker (integração), fuzz de permissão (`Back.ApiTests` no GeoCloud — padrão a estender).
...
Os produtos ativos são `GeoCloudAI` e `GeoCloudAI`. O `geocloud-ai-framework`
