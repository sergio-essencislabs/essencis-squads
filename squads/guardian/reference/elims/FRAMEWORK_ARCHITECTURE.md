...
O `elims-ai-framework` é um projeto de conhecimento e processo — não um projeto de código de produto. Ele existe para ser lido por agentes de IA (e humanos) antes e durante o trabalho em ELIMS, e para capturar de volta o que foi aprendido.
...
3. **Independência de produto** — o framework não depende de estar dentro de nenhum produto específico; ele referencia ELIMS por caminho relativo, nunca os incorpora.
...
| `parity-diff` | Compara núcleo compartilhado entre ELIMS. |
...
| `framework-sync` | Sincroniza `elims-ai-framework/` com `.cursor/rules/` e `.cursor/skills/` do workspace. |

## Fluxo de contexto

```mermaid
flowchart LR
    subgraph Fonte["Fonte versionada (elims-ai-framework/, git próprio)"]
      P[policies/]
      S[skills/]
      AG[agents/]
    end
    subgraph DeployGeoCloud["Deploy (ELIMS/.cursor/, gerado)"]
      PR1[rules/*.mdc]
      SR1[skills/*/SKILL.md]
    end
    subgraph DeployGeoCloud["Deploy (ELIMS/.cursor/, gerado)"]
    ...
    PR1 -- alwaysApply / globs --> CursorGeoCloud[Cursor Agent — janela ELIMS]
    SR1 -- nome/descrição --> CursorGeoCloud
    PR2 -- alwaysApply / globs --> CursorGeoCloud[Cursor Agent — janela ELIMS]
    SR2 -- nome/descrição --> CursorGeoCloud
```

O `elims-ai-framework` tem remoto próprio no GitHub (`sergio-essencislabs/elims-ai-framework`). Os produtos
(`ELIMS`, `ELIMS`) são repositórios separados, cada um aberto em
...
`ELIMS` estar aberta como workspace.
...
- Git próprio (`elims-ai-framework/.git`), independente dos repositórios de produto
  (`ELIMS`, `ELIMS`). Remoto do framework:
  `github.com/sergio-essencislabs/elims-ai-framework`.
...
- A skill `structural-spreadsheet-sync` mantém a planilha estrutural canônica
  (`Documentation/Main/Planilha_ELIMS_main.xlsx`) sincronizada com o mesmo
  gatilho — somente a branch `main`, no mesmo padrão do GeoCloud.
