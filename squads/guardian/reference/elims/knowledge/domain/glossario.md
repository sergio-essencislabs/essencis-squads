...
Herança do código legado do ELIMS (comentários/nomes antigos em português) mapeado para os nomes em inglês usados no código atual. Nunca reintroduzir a coluna PT em código novo — ver `policies/nomenclatura.md`.
...
| Parity | Ação de manter ELIMS consistente com o núcleo do ELIMS (classes + permissões). |
| Drift | Divergência não intencional entre código e banco, ou entre ELIMS. |
