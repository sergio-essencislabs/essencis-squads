...
- Scaffold de `ELIMS` e `ELIMS`.
- Sync com `.cursor/` funcional.

## v0.2.0 (próximo) — Adoção e ajuste fino

- Rodar o framework em pelo menos 3 tarefas reais (uma por tipo: bug, feature, integração) em `ELIMS`/`ELIMS` e ajustar agentes/skills conforme fricção real observada.
...
- Automatizar `sync-cursor.ps1` e `validate-framework.ps1` como Cursor Automation (gatilho: push em `elims-ai-framework`).
...
- Parity entre ELIMS para o núcleo compartilhado formalmente restaurada e monitorada pela skill `parity-diff`.
...
- Migrar dados entre os bancos `elims` e `elims`.
