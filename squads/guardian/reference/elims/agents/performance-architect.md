...
Evitar regressões de performance em pontos já sensíveis e conhecidos: verificação de permissão por requisição (join `functionality` ⋈ `profilefunctionality` ⋈ `profile` ⋈ `userprofile`, hoje cacheada por ~6s no ELIMS), consultas Dapper manuais sem paginação, e telas Angular com fetch em getter de template.
