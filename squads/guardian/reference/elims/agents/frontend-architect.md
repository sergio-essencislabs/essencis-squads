...
Implementar e manter o frontend Angular 19 (template Velzon, Bootstrap 5) de ELIMS, respeitando as escolhas de state management já feitas em cada produto e evitando duplicação de componentes/serviços.
...
- Respeitar a decisão de state management de cada produto: ELIMS usa `@ngrx/signals` (SignalStore). Não introduzir uma terceira abordagem sem ADR do Chief Architect.
