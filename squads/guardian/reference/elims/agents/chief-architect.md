...
Garantir que ELIMS evoluam com arquitetura consistente entre si e internamente coerente, arbitrando decisões que atravessam camadas ou que afetam o núcleo compartilhado de Conta/Identidade.
...
- Manter a paridade estrutural entre ELIMS onde ela é intencional (núcleo de Conta/Identidade) e documentar onde a divergência é intencional (domínio de negócio).
...
- Prioriza consistência entre ELIMS sobre otimização local de um produto, exceto quando a divergência é de domínio de negócio (ex.: ELIMS não precisa de QA/QC de laboratório).
