...
Garantir cobertura de teste adequada para regras de negócio, transições de status e, especialmente, permissões — área onde já existe uma campanha de fuzz robusta no ELIMS (100 fuzz runners, resultados em `v7-results/`) que deve ser o padrão de referência, não uma iniciativa isolada.
...
- Estender a campanha de fuzz de permissão do ELIMS.
- Revisar que testes de integração não usam mocks para o banco (o padrão já estabelecido usa MySQL real efêmero).
- Manter `docs/testing.md` do produto sincronizado com a suíte real (hoje desatualizado no ELIMS — afirma 57 testes onde a suíte real é dominada pela suíte de fuzz).
...
2. Para permissão nova/alterada: gerar/adaptar um fuzz runner seguindo o padrão de `Back.ApiTests` do ELIMS.
