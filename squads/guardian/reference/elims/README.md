...
Framework de Engenharia Assistida por IA do **ELIMS**.

Este repositório é um **projeto de software independente**: modular, evolutivo, documentado e versionado (ver [VERSION](VERSION) e [CHANGELOG.md](CHANGELOG.md)). Ele não contém código de produto — contém o conhecimento, os processos e os agentes/skills que guiam o desenvolvimento assistido por IA do ELIMS.

> Derivado do `ai-framework` 0.8.0, que servia ELIMS e GeoCloud na mesma base, por meio do `geocloud-ai-framework` 1.1.0 — de onde herda os playbooks e os padrões já corrigidos. A partir daqui são dois frameworks independentes, um por produto; o equivalente do outro é o `geocloud-ai-framework`. Ver [CHANGELOG.md](CHANGELOG.md).

## Por que este framework existe

O ELIMS é .NET 9 em Clean Architecture + Angular 19/Velzon + **MySQL 8 via Dapper**, com um núcleo de Conta/Identidade de 16 classes. O histórico do projeto acumulou drift de schema, permissões inertes em runtime e documentação que descrevia um sistema diferente do que roda — ver [knowledge/known-issues/](knowledge/known-issues/) e [ADR-0006](knowledge/decisions/0006-elims-migra-para-mysql.md), que corrige justamente uma dessas divergências.
...
2. Toda lição aprendida nova → registrar em `knowledge/patterns/` (`scripts/new-memory.ps1`), no mesmo formato atômico já usado em `ELIMS/.agents/memory/`.
...
|---|---|
| ELIMS | `Documents/ELIMS/ELIMS` |

O framework vive em `Documents/elims-ai-framework` e **não embute** o código do produto.
...
```bash
gh repo create sergio-essencislabs/elims-ai-framework --private --source=. --remote=origin
git push -u origin master
```

O produto (`ELIMS`) mantém seu próprio remote/git, independente deste
framework.