...
- `UserService.Register` (ELIMS) encadeando Account→Entity→Profile→User→UserProfile sem transação nem método auxiliar — sinalizado como candidato a extração de método + risco de rollback parcial (já catalogado como known-issue).
