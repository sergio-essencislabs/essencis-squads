...
7. Para cada PR **aprovado**, checar as 3 evidências de fechamento na `GT-NNNN.md` correspondente, nesta ordem: (a) **código** — "Registro de execução → Alterações realizadas" preenchido e PR de fato existente; (b) **documentação** — se a task tocou módulo/endpoint/schema/integração, o doc correspondente foi atualizado (ou há justificativa explícita registrada de por que não se aplica); (c) **testes** — "Validação" tem comando e resultado reais, não uma caixa marcada sem evidência. Se as três passam, mover **os dois arquivos** do par: `tasks/active/GT-NNNN.md` do hub **e** `<repo>/.agents/tasks/active/GT-NNNN.md`, ambos → `completed/`, com `status: completed` no front-matter de cada um. Mover só um deixaria o par divergente já no primeiro fechamento, que é exatamente o que a `contraparte` existe para impedir. As três evidências são checadas **no par do repositório de produto**, que é onde o Registro de execução foi escrito; se a task não tem `contraparte`, tudo no hub. Se alguma falhar, **não mover** — registrar exatamente o que falta no campo "Pendências" da própria task, nunca em silêncio.
...
7. Um lado do par foi movido para `completed/` e o outro não — os dois andam juntos, ou nenhum anda.
