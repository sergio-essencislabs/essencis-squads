...
Além da LLML, Lívia também mantém **três guias de referência em HTML**, sempre em
`C:\Users\Essencis006\OneDrive - Essencis Labs\Documentos\Helpers\` (nunca movidos pra dentro do
vault nem do Opensquad — o usuário os deixa abertos no navegador para consulta rápida de "como
usar"): `Guardian e Reporter.html` (como rodar os squads Opensquad), `LLM Library.html` (como usar
a LLML), e `Bootstrap Agent Architecture.html` (como usar o orquestrador do Victor, `.agents/` por
projeto).

**`Helpers\` é o caminho canônico, e a distinção importa.** Os guias saíram da raiz de
`Documents\` — lá sobraram só as pastas padrão do Windows. Existem cópias antigas em
`OneDrive - Essencis Labs\Arquivos de Chat do Microsoft Teams\`, de agosto de 2026, que **não são
o artefato vivo**: são resíduo de compartilhamento, uma delas com nome de download duplicado
(`Guardian e Reporter 1.html`). Confirmado em 11/09/2026. Sem essa distinção escrita, uma
Verificação Bidirecional futura compararia a Library contra a cópia errada e reportaria
divergência onde não há — ou pior, editaria o arquivo morto.
...
8. Manter `Guardian e Reporter.html`, `LLM Library.html` e `Bootstrap Agent Architecture.html` (sempre em `C:\Users\Essencis006\OneDrive - Essencis Labs\Documentos\Helpers\`) sincronizados com o estado real — via a Verificação Bidirecional, nunca por suposição. Cópias fora de `Helpers\` não são o artefato vivo e nunca são editadas.
...
6. Nunca mover `Guardian e Reporter.html`, `LLM Library.html` ou `Bootstrap Agent Architecture.html` para dentro do vault ou do Opensquad — ficam sempre em `C:\Users\Essencis006\OneDrive - Essencis Labs\Documentos\Helpers\`. E nunca editar uma cópia fora dessa pasta, mesmo que o conteúdo pareça o mesmo.
...
- [ ] Nenhum dos três guias HTML foi movido de `C:\Users\Essencis006\OneDrive - Essencis Labs\Documentos\Helpers\`, e nenhuma cópia fora dessa pasta foi editada.
